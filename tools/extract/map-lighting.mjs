...
import { readBspPak, readBspPakEntry } from '../lib/bsp-pak.mjs';
...
function extractMapCubemap(map, bspPath, position) {
  const pak = readBspPak(bspPath);
  const mapPattern = map.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const pattern = new RegExp(`^materials/maps/${mapPattern}/c(-?\\d+)_(-?\\d+)_(-?\\d+)\\.vtf$`, 'i');
  const candidates = pak.entries.flatMap((entry) => {
    const match = entry.name.match(pattern);
    if (!match) return [];
    return [{ entry, origin: match.slice(1).map(Number) }];
  });
  if (candidates.length === 0) throw new Error(`No LDR map cubemaps found in ${bspPath}`);
  candidates.sort((a, b) => distanceSquared(a.origin, position) - distanceSquared(b.origin, position));

  const selected = candidates[0];
  const decoded = decodeVTFCubemap(readBspPakEntry(pak.data, selected.entry));
  if (decoded.length !== CUBEMAP_FACES.length || decoded.some((face) => face.width !== face.height)) {
    throw new Error(`Map cubemap is not a square six-face texture: ${selected.entry.name}`);
  }
  const outputDir = path.join(MAP_CUBEMAP_OUT, map, selected.origin.join('_'));
  fs.mkdirSync(outputDir, { recursive: true });
  decoded.forEach((face, index) => {
    fs.writeFileSync(
      path.join(outputDir, `${CUBEMAP_FACES[index]}.png`),
      encodePNG(face.rgba, face.width, face.height),
    );
  });
...
function main() {
  ...
  for (const preset of PRESETS) {
    ...
    const cubemapOrigin = extractMapCubemap(preset.map, bspPath, modelOrigin);
  ...
  console.log(`Wrote ${path.relative(ROOT, GENERATED_OUT)}, ${PRESETS.length} skyboxes, and ${PRESETS.length} map cubemaps.`);
}
