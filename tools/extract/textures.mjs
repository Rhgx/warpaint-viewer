...
export async function extractAndDecodeTextures({
  ...
  for (const item of wanted) {
    ...
    try {
      ...
      if (!force && item.outExists && prevHashes[item.pub] === hash && previousMetadata[item.pub]) {
        metadata[item.pub] = previousMetadata[item.pub];
      ...
      await sharp(decoded.rgba, { raw: { width: decoded.width, height: decoded.height, channels: 4 } })
        .webp({ lossless: true, effort: 4, exact: true })
        .toFile(outputPath);
      if (item.outExists) reencoded++; else brandNew++;
      if ((reencoded + brandNew) % 200 === 0) log(`  ... ${reencoded + brandNew} (re)encoded`);
    } catch (error) {
      failed++;
