import { fixturePaintkitIds as builtInIds } from '../fixtures';
...
const exported = await exportEditedPackage(edited, { package: sourcePackage, name: 'Edited Paint' });
...
const normalized = normalizeProtoDefFragments([
...
const minimal = await exportEditedPackage(edited, { name: 'Standalone Paint' });
...
if (fs.existsSync(fullPath) && fs.existsSync(itemDefsPath) && fs.existsSync(manifestPath)) {
  ...
  const decoded = decodeProtoDefs(fullBytes, {
  ...
  const vpdExport = await exportEditedPackage(editedContainerKit, { package: vpdPackage });
  ...
  const rewritten = decodeProtoDefs(bytesFile(vpdOutput, 'scripts/protodefs/proto_defs.vpd'), {
    weaponsByItemDef: itemDefs,
    builtInIds: builtInIds(manifest),
  });
  assert.match(
    JSON.stringify(extractKitMessages(rewritten, 435)?.operation),
