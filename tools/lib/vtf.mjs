...
export function parseVTFSpriteSheet(buf) {
  ...
  try {
    // The resource directory offset points at a 4-byte length-prefix word (the size of the
    // blob that follows), not at the blob itself - skip it to reach the actual sheet data.
    return parseSheetBlob(buf, off + 4);
  } catch {
    return null;
  }
}
